...
def read_h5_data(path):
    with h5py.File(path, 'r') as hf:
        data = np.array(hf.get('patch_image'))
    return data
...
def input2patches(image, config):
    patches = []
    h, w = image.shape
    nx=0
    ny=0
    for i in xrange(0,h-config.image_size+1, config.stride):
        nx+=1;ny=0
        for j in xrange(0,w-config.image_size+1, config.stride):
            ny+=1
            patch = image[i:i+config.image_size, j:j+config.image_size]
...
def fit_scale_crop(hr, raw):
    h_hr,w_hr = hr.shape
    h_raw, w_raw = raw.shape
    h, w=min(h_hr, h_raw), min(w_raw, w_hr)
    hr = hr[0:h, 0:w]
    raw = raw[0:h,0:w]
    return hr
def ycbcr2rgb(im):
